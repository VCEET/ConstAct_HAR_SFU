...
#from smac.facade.smac_hpo import SMAC4HPO
  ...
  #      self.load_model()
    ...
    def load_model(self,params):
        ...
        for name in self.arg.model_args['head']:
            ...
            self.prompt_learner2 = PromptLearner(class_names, self.model_2,params['prompt_length'],params['class_position']).cuda(self.output_device)
            ...
            ("linear2", nn.Linear(512 // 16, params['prompt_length']))
    ...
    def load_optimizer(self,params):
        ...
        if self.arg.optimizer == 'AdamW':
            ...
            self.optimizer = optim.AdamW([{'params': self.skeleton_encoder.parameters(),'lr': params['learning_rate']*10},
                 {'params': self.visual_encoder.parameters(),'lr': params['learning_rate']*10},
                 {'params': self.model_.visual.parameters(), 'lr': params['learning_rate']},
                 {'params': self.fusion.parameters(), 'lr':params['learning_rate']*10},
                 {'params': self.model_text_2.parameters(), 'lr':params['learning_rate']},
                 {'params': self.prompt_learner2.parameters(), 'lr':params['learning_rate']*10},
                 {'params': self.meta_net_skeleton.parameters(), 'lr':params['learning_rate']*10},],
                betas=(0.9, 0.98),lr=params['learning_rate'], eps=1e-8,
    ...
    def adjust_learning_rate(self, epoch, params):
        if self.arg.optimizer == 'SGD' or self.arg.optimizer == 'AdamW':
            if epoch < self.arg.warm_up_epoch:
                lr = params['learning_rate']* (epoch + 1) / self.arg.warm_up_epoch
            else:
                lr = params['learning_rate'] * (
                        params['lr_decay_rate'] ** np.sum(epoch >= np.array(self.arg.step)))
    ...
    def train(self, params, epoch, save_model=False):
        ...
        self.adjust_learning_rate(epoch,params)
            ...
            with torch.cuda.amp.autocast():
                ...
                for ind in range(num_text_aug):              
                    ...
                    if ind > 0:
                        ...
                        self.prompts2[ind,label][:,1:1+params['prompt_length']] += skeleton_prompt_feat
                    else:
                ...
                loss = loss_ce + params['loss_alpha']*loss_pose
    ...
    def objective_function(self, params,seed=None):
        self.load_data(params)
        self.load_model(params)
        self.load_optimizer(params)
        total_eval_loss = 0
        for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
            save_model = (((epoch + 1) % self.arg.save_interval == 0) or (
                    epoch + 1 == self.arg.num_epoch)) and (epoch+1) > self.arg.save_epoch

            self.train(params,epoch, save_model=save_model)
            eval_loss = self.eval(params,epoch, save_score=self.arg.save_score, loader_name=['test'])
    ...
    def start(self):
        ...
        learning_rate = UniformFloatHyperparameter('learning_rate', lower=1e-7, upper=1e-1, default_value=self.arg.base_lr, log=True)
        batch_size = UniformIntegerHyperparameter('batch_size', lower=self.arg.batch_size/8, upper=self.arg.batch_size*2, default_value=self.arg.batch_size)
        weight_decay = UniformFloatHyperparameter('weight_decay', lower=0.0, upper=self.arg.weight_decay*10, default_value=self.arg.weight_decay)
        prompt_length = UniformIntegerHyperparameter('prompt_length', lower=1, upper=40, default_value=self.arg.prompt_length)
        class_position = CategoricalHyperparameter('class_position', ['middle','start','end'], default_value=self.arg.class_position)
        loss_alpha = UniformFloatHyperparameter('loss_alpha', lower=0.0, upper=1.0, default_value=self.arg.loss_alpha)
        lr_decay_rate = UniformFloatHyperparameter('lr_decay_rate', lower=0.0, upper=1.0, default_value=self.arg.lr_decay_rate)

        cs.add_hyperparameters([learning_rate, weight_decay, prompt_length, batch_size, class_position, loss_alpha, lr_decay_rate])

        scenario = Scenario(cs, deterministic=True, n_trials=20)
        ...
        if self.arg.phase == 'train':
            ...
            ## for SMAC
            smac = HyperparameterOptimizationFacade(scenario, self.objective_function)
