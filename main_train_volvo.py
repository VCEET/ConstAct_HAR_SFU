...
#classes_visual, num_text_aug_visual, text_dict_visual = text_visual_prompt_with_object()
...
#        print(self.skeleton_encoder)
        ...
        for name in self.arg.model_args['head']:
           ...
           # self.model_text_2 = TextCLIP(self.model_2)
 ...
 #           self.tokenized_prompts = self.prompt_learner.tokenized_prompts.cuda(self.output_device)
 #           self.prompts = self.prompt_learner()

            self.prompt_learner2 = PromptLearner(class_names, self.model_2,self.arg.prompt_length,self.arg.class_position).cuda(self.output_device) #PromptLearner(class_names, self.model_2,self.arg.prompt_length).cuda(self.output_device)
            ...
            ## Added for prompt learning CoCoOp
   ...
   #              {'params': text_params,'lr': self.arg.base_lr},
  ...
  #               {'params': self.prompt_learner.parameters(), 'lr':self.arg.base_lr*10},
   ...
   #     self.meta_net_visual.train()
        ...
        for batch_idx, (data, label, index, RGB_images, data_bone) in enumerate(process):       
            ...
            with torch.no_grad():
                ...
                data_bone = data_bone.float().cuda(self.output_device)
    ...
    #            image_features_prompt = image_embedding / image_embedding.norm(dim=-1, keepdim=True)
    #            img_prompt_feat = self.meta_net_visual(image_features_prompt).unsqueeze(-1)
    #            self.prompts[label][:,1:1+self.arg.prompt_length] += img_prompt_feat
  ...
  #              text_embedding_visual = self.model_text_dict[self.arg.model_args['head'][0]](texts_visual).float()
    ##            text_embedding_visual = self.model_text_dict[self.arg.model_args['head'][0]](self.prompts[label],self.tokenized_prompts[label]).float()
...
#                        self.prompts2[ind,label] += skeleton_prompt_feat
...
#                        self.prompts2[ind,label] += skeleton_prompt_feat
                        ...
                        # texts_skeleton = torch.cat(texts).cuda(self.output_device)
             ...
             #       text_embedding_skeleton_list.append(self.model_text_dict_2[self.arg.model_args['head'][0]](texts_skeleton).float())
...
#                loss = loss_ce + self.arg.loss_alpha*loss_visual + self.arg.loss_alpha*loss_pose*4
...
#            loss_visual_value.append(loss_visual.item())
 ...
 #           state_dict_model_text = self.model_text.state_dict()
  ...
  #          weights = OrderedDict([[k.split('module.')[-1], v.cpu()] for k, v in state_dict_model_text.items()])
  #          torch.save(weights, epoch_path +  '/model_text-RGB' + '.pt')
