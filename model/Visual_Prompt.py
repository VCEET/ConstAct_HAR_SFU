...
class visual_prompt(nn.Module):
    def __init__(self, sim_head, clip_state_dict, T):
        ...
        self.reduce_feat1 = nn.Linear(16,8)
        self.reduce_feat2 = nn.Linear(16,4)
        self.reduce_feat3 = nn.Linear(16,2)
        self.reduce_feat4 = nn.Linear(16,1)

        self.feat_list = nn.ModuleList()
        for i in range(4):
            self.feat_list.append(nn.Linear(512,512))
   ...
   #     reduced_embeddings1 = self.reduce_feat1(x.clone()).mean(dim=2,keepdim=False)
   #     reduced_embeddings2 = self.reduce_feat2(x.clone()).mean(dim=2,keepdim=False)
   #     reduced_embeddings3 = self.reduce_feat3(x.clone()).mean(dim=2,keepdim=False)
   #     reduced_embeddings4 = self.reduce_feat4(x.clone()).mean(dim=2,keepdim=False)
