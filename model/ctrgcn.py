...
class Model_lst_4part(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        for i in range(4):
            self.part_list.append(nn.Linear(1024,512))

        self.change_embed = nn.Linear(1024,256)
        self.head = head

        if 'ViT-B/32' in self.head:
            self.linear_head['ViT-B/32'] = nn.Linear(1024,512)
            conv_init(self.linear_head['ViT-B/32'])
        
        if 'ViT-B/16' in self.head:
            self.linear_head['ViT-B/16'] = nn.Linear(1024,512)
    ...
    def forward(self, x):
        ...
        x = x.view(N, M, c_new, -1)
        embeddings = x.mean(1)
        embeddings = self.change_embed(embeddings.permute(0,2,1)).permute(0,2,1)
        x = x.mean(3).mean(1)
...
#        return self.fc(x), feature_dict, self.logit_scale, [head_feature, hand_feature, hip_feature, foot_feature]
        return embeddings, feature_dict, self.logit_scale, [head_feature, hand_feature, hip_feature, foot_feature]
...
class Model_lst_4part_mix(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
                 drop_out=0, adaptive=True, head=['ViT-B/32'], k=0):
        super(Model_lst_4part_mix, self).__init__()
        ...
        self.num_class = num_class
        self.num_point = num_point
        self.data_bn = nn.BatchNorm1d(num_person * in_channels * num_point * 2)
    ...
    def forward(self, x, x_bone):

        x = torch.cat((x,x_bone),dim=4)
      #  print(x.shape)
        if len(x.shape) == 3:
            N, T, VC = x.shape
            x = x.view(N, T, self.num_point, -1).permute(0, 3, 1, 2).contiguous().unsqueeze(-1)
...
class Model_lst_4part_same(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
                 drop_out=0, adaptive=True, head=['ViT-B/32'], k=0):
        super(Model_lst_4part_same, self).__init__()
    ...
    def forward(self, x, x_bone):
        ...
        x = x.view(N, M, c_new, -1)
        joint_embeddings = x[:,:2].mean(1)
        bone_embeddings = x[:,2:].mean(1)

        embeddings = x.mean(1)
        x = x.mean(3).mean(1)

        feature_dict = dict()
        feature_joint_dict = dict()
        feature_bone_dict = dict()
        ...
        x = self.drop_out(x)
#        return self.fc(x), feature_dict, self.logit_scale, [head_feature, hand_feature, hip_feature, foot_feature]
        return embeddings, joint_embeddings, bone_embeddings, feature_dict, self.logit_scale, [head_feature, hand_feature, hip_feature, foot_feature]
